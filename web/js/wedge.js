...
scene.add(wedge); 
    ...
    wedge.attr = "name,length,breadth,width,color,opacity,material,rotate";
    wedge.status=-1;
    wedge.obj="wedge";
    objects.push(wedge);
    count+=1;
    dragElements.push(wedge);
    collidableMeshList.push(wedge);
    wedge.parameters = 
    {
        name : wedge.name,
        length: 2,
        ...
        material: "Phong",
        rotate:0,
        delete:function(){ wedge.status=0;scene.remove(wedge) ;gui.destroy();gui=new dat.GUI();}
    };
    wedge.gui=function (wedge)
    {
        ...
        var wedgeRotate=gui.add(wedge.parameters,'rotate').min(-Math.PI).max(Math.PI).step(0.1).name('Rotate').listen();
        ...
        gui.add( wedge.parameters, 'delete' ).name("delete");
    ...
    {
       wedge.name=wedge.parameters.name;
       wedge.material.color.setHex( wedge.parameters.color.replace("#", "0x") );
       var A = new THREE.Vector2( 0, 0 );
        var B = new THREE.Vector2( wedge.parameters.breadth, 0 );
        var C = new THREE.Vector2( wedge.parameters.breadth, wedge.parameters.length);
        ...
        wedge.geometry=new PrismGeometry( [ A, B, C ], wedge.parameters.width ); 
        wedge.geometry.needVerticesUpdate = true;
        wedge.rotation.y=wedge.parameters.rotate;
        wedge.material.opacity = wedge.parameters.opacity;
        updateMaterial(wedge,wedge.parameters.material); 
    };
