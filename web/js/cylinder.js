...
function cylinder_creator(radius=5,height=5){
    ...
    cylinder.attr = "name,radius,height,color,opacity,material,rotateX,rotateY,rotateZ";
    cylinder.status=-1;
    ...
    cylinder.parameters = 
    {
        ...
        rotateY:0,
        rotateZ:0,
         delete:function(){ cylinder.status=0;scene.remove(cylinder) ;gui.destroy();gui=new dat.GUI();},
    ...
    cylinder.gui=function(cylinder)
  {
    ...
    gui.add( cylinder.parameters, 'rotateZ' ).name("Rotate in Z");*/
        gui.add( cylinder.parameters, 'delete' ).name("Delete");
    gui.open();

  };
  cylinder.update=function(){
      cylinder.name=cylinder.parameters.name;
      cylinder.material.color.setHex( cylinder.parameters.color.replace("#", "0x") );
      cylinder.geometry = new THREE.CylinderGeometry(cylinder.parameters.radius,cylinder.parameters.radius,cylinder.parameters.height, 500, false);
      cylinder.geometry.needVerticesUpdate = true;
      cylinder.material.opacity = cube.material.opacity;   
      updateMaterial(cylinder,cylinder.parameters.material);
      cylinder.rotation.x=cylinder.parameters.rotateX;
      cylinder.rotation.y=cylinder.parameters.rotateY;
      cylinder.rotation.z=cylinder.parameters.rotateZ;
  };
    return cylinder;
}
