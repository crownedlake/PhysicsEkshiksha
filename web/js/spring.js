...
function spring_creator(coilR=2,coilHeight=10)
{
    ...
    spring.attr = "name,radius,height,color,opacity,material,rotate";
    spring.obj="spring";
    spring.status=-1;
    objects.push(spring);
    count+=1;
    dragElements.push(spring);
    collidableMeshList.push(spring);
    spring.parameters = 
    {
        name : spring.name,
        radius:2,
        ...
        material: "Phong",
        rotate:0,
        delete:function(){ spring.status=0;scene.remove(spring) ;gui.destroy();gui=new dat.GUI();}
    ...
    spring.gui=function (spring)
    {
        ...
        gui.add( spring.parameters, 'delete' ).name("Delete");
	...
	spring.update=function()
        {
           spring.name=spring.parameters.name;
           spring.material.color.setHex( spring.parameters.color.replace("#", "0x") );  
           value=spring.parameters.height;
            if(spring.spring2.length>value)
            {
                for(var i=spring.spring2.length-1;i>=value;i--)
                {
            ...
            spring.scale.set(spring.parameters.radius/2,spring.parameters.radius/2,spring.parameters.radius/2);
            spring.material.opacity = spring.parameters.opacity;
            updateMaterial(spring,spring.parameters.material); 
            spring.rotation.x=spring.parameters.rotate;
        };
	return spring;
}