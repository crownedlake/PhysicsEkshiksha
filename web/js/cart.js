...
function cart_creator(){
	...
	scene.add(carModel);
        ...
        carModel.attr = "name,size";
        carModel.status=-1;
        carModel.obj="cart";
        objects.push(carModel);
        count+=1;
        dragElements.push(carModel);
        collidableMeshList.push(carModel);
        carModel.obj="cart";
        ...
        carModel.parameters = 
        {
            name : carModel.name,
            size:1,
            delete:function(){ carModel.status=0;scene.remove(carModel) ;gui.destroy();gui=new dat.GUI();},
        ...
        carModel.gui=function (carModel)
        {
            ...
            gui.add( carModel.parameters, 'delete' ).name("delete");
            gui.open();

        };
        carModel.update=function(){
            carModel.name=carModel.parameters.name;
            carModel.scale.set(carModel.parameters.size,carModel.parameters.size,carModel.parameters.size); 
