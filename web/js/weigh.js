...
function weigh_creator(unit){
	...
	scene.add(weigh);
        ...
        collidableMeshList.push(weigh);
        weigh.parameters = 
        {
            name : weigh.name,
            scale:1,
            dial:0,
            delete:function(){ weigh.status=0;scene.remove(weigh) ;gui.destroy();gui=new dat.GUI();}
        ...
        weigh.gui=function (weigh)
        {
            ...
            gui.add(weigh.parameters,'delete').name('Delete');
            gui.open();

        };
        weigh.update=function()
        {
            weigh.name=weigh.parameters.name;
            weigh.scale.set(weigh.parameters.scale,weigh.parameters.scale,weigh.parameters.scale);
            weigh.dial.position.set(0.5*Math.sin(weigh.parameters.dial*Math.PI/8),0.5*Math.cos(weigh.parameters.dial*Math.PI/8),0.1);
            weigh.dial.rotation.z=-weigh.parameters.dial*Math.PI/8;
        };
	return weigh;
